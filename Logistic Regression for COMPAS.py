...
evaluate_model("Logistic Regression", y_pred, race_test)
...
evaluate_model("Exponentiated Gradient", y_pred_expgrad, race_test)
...
for i in np.random.choice(len(X_test), 3, replace=False):
    ...
    print("\n[Exponentiated Gradient + Logistic Regression]")
...
evaluate_model("ThresholdOptimizer (Equalized Odds)", y_pred_post, race_test)
...
for i in np.random.choice(len(X_test), 3, replace=False):
    ...
    print("\n[ThresholdOptimizer + Logistic Regression]")
...
for opt in sensitive_options:
    ...
    evaluate_model(f"Reweighing on {opt['name']} (Logistic Regression)", y_pred_rw, sensitive_features)
    ...
    def predict_proba_baseline(X_input):
        ...
        X_base = X_df[baseline_cols].to_numpy()
        return lr.predict_proba(X_base)


    # Reweighing Predictive function
    def predict_proba_rw(X_input, protected_values=None):
        X_df = pd.DataFrame(X_input, columns=all_features)
        if protected_values is not None:
            for p in opt["protected"]:
                X_df[p] = protected_values[p]
        X_rw = X_df[all_features].to_numpy()
        return lr_rw.predict_proba(X_rw)
    ...
    for i in np.random.choice(len(test_df), 3, replace=False):
        ...
        print("\n[Reweighing + Logistic Regression]")

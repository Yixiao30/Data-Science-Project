...
# ----------------- Construct sensitive attributes -----------------
...
sensitive_df['residence_since'] = X['present_residence_since'].astype(str)  # Length of Residence
sensitive_df['housing'] = X['housing'].astype(str)                          # Housing
sensitive_df['savings_status'] = X['savings_status'].astype(str)            # Savings Account
sensitive_df['checking_status'] = X['checking_status'].astype(str)          # Checking Account
sensitive_df['property'] = X['property'].astype(str)                        # Owning Assets
...
# ----------------- Preprocessing: Numerical standardization + category one-hot -----------------
...
evaluate_and_print("Baseline LogisticRegression", y_test, y_pred_baseline, sens_test_df['personal_status_sex'])
...
# Transform the classification features using OrdinalEncoder
oe = OrdinalEncoder()
...
# fit threshold optimizer on training set (with sensitive features)
...
# train logistic with instance weights
lr_rw = LogisticRegression(max_iter=1000, solver='liblinear')
lr_rw.fit(train_rw.features, train_rw.labels.ravel(), sample_weight=train_rw.instance_weights)

y_pred_rw = lr_rw.predict(test_aif.features)
# evaluate by original textual groups
